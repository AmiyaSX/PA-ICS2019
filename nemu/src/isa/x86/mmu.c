...
uint32_t page_translate(uint32_t addr)//am.h,vme.c,mmu.c
{
	...
	/*PDE *pdp=(PDE*)((uintptr_t)(PTE_ADDR(cpu.cr3.val)));//找到页目录入口地址
	PDE pde=pdp[Dir];//？找到对应的页目录项
	if (pde.present==0) assert(0);
	PTE *ptp=(PTE*)(PTE_ADDR(pde.val));
	PTE pte=ptp[Page];
	if (pte.present==0) assert(0);*/
	//PDE *pdp=(PDE*)(PTE_ADDR(cpu.cr3.val));//找到页目录入口地址
	PDE pde;
	pde.val=paddr_read(PTE_ADDR(cpu.cr3.val)|(Dir<<2),4);//？找到对应的页目录项
	if (pde.present==0) assert(0);
	PTE pte;
...
uint32_t isa_vaddr_read(vaddr_t addr, int len) {
	if (cpu.cr0.paging==0)//get pg
		return paddr_read(addr, len);
	int s=addr/PAGE_SIZE,t=(addr+len-1)/PAGE_SIZE;
	if (s!=t)//data cross the page
	{
		uint32_t ret=0;
